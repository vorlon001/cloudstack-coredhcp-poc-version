...
type DhcpListenRunner func(Listiner string, configByte []byte, logs *logrus.Entry) error
...
func connect(task *Task) {
        ...
        err = task.DhcpListenRunner(task.ListenName, yamlData, task.Logs)
        if err != nil {
...
func RunJob(Listiner string, configByte []byte, logs *logrus.Entry) error {

        config, err := config.Load(Listiner, "yml", configByte, *flagConfig)
        if err != nil {
		return errors.New(fmt.Sprintf("Failed to load configuration: %v", err))
        }
        // register plugins
	...
	var RegisteredPlugins = make(map[string]*plugins.Plugin)
        ...
        for _, plugin := range desiredPlugins {
                if err := plugins.RegisterPlugin(Listiner, RegisteredPlugins, plugin); err != nil {
			return errors.New(fmt.Sprintf("Listiner: %s, Failed to register plugin '%s': %v", Listiner, plugin.Name, err))
                }
        }

        // start server
        srv, err := server.Start(Listiner, config, RegisteredPlugins)
        if err != nil {
		return errors.New(fmt.Sprintf("Listiner: %s, Failed'%s'", Listiner, err))
        }
...
func main() {
        ...
        for i, v := range config.Config {
                taskListen := Task{ListenName: i, Listen: &v, Uuid: uuid.NewString(), ErrChannel: errChannel, Wg: &wg, DhcpListenRunner: RunJob, Logs: log}
                task <- &taskListen
        }
