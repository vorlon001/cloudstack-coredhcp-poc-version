...
var (
	v6ServerID map[string]dhcpv6.DUID
	v4ServerID map[string]net.IP
)

func init() {

        v6ServerID = make(map[string]dhcpv6.DUID)
        v4ServerID = make(map[string]net.IP)
...
func Handler6(Listiner string, req, resp dhcpv6.DHCPv6) (dhcpv6.DHCPv6, bool) {
	...
	if sid := msg.Options.ServerID(); sid != nil {
		...
		if !sid.Equal(v6ServerID[Listiner]) {
	...
	dhcpv6.WithServerID(v6ServerID[Listiner])(resp)
	return resp, false
}
...
func Handler4(Listiner string, req, resp *dhcpv4.DHCPv4) (*dhcpv4.DHCPv4, bool) {
	...
	if req.ServerIPAddr != nil &&
		...
		!req.ServerIPAddr.Equal(v4ServerID[Listiner]) {
	...
	copy(resp.ServerIPAddr[:], v4ServerID[Listiner])
	resp.UpdateOption(dhcpv4.OptServerIdentifier(v4ServerID[Listiner]))
	return resp, false
}
...
func setup4(Listiner string, args ...string) (handler.Handler4, error) {
	...
	v4ServerID[Listiner] = serverID.To4()
	return Handler4, nil
}
...
func setup6(Listiner string, args ...string) (handler.Handler6, error) {
	...
	case "ll", "duid-ll", "duid_ll":
		v6ServerID[Listiner] = &dhcpv6.DUIDLL{
			// sorry, only ethernet for now
			HWType:        iana.HWTypeEthernet,
			LinkLayerAddr: hwaddr,
		}
	case "llt", "duid-llt", "duid_llt":
		v6ServerID[Listiner] = &dhcpv6.DUIDLLT{
